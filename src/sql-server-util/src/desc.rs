...
impl SqlServerTableDesc {
    ...
    /// upstream table described by `other`, a fresh unmodified description of
    /// the same table.
    ///
    /// `self` is the description the export was created with, after `EXCLUDE
    /// COLUMNS` was applied. Added constraints are compatible. Constraints
    /// spanning an excluded column are not recorded as keys, see
    /// `purify_source_exports` in `mz_sql`, so nothing depends on them. Column
    /// changes are outside this check, which only decides whether a key
    /// Materialize relies on still exists upstream.
